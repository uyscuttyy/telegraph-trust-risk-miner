...
const expected: EvaluationCase = { id: "case", intent: "URL_SCAN", input: { intent: "URL_SCAN", url: "https://example.com" }, groundTruth: { label: "safe", risk: 10, confidence: .8, rationale: "fixture" }, provenance: { sourceType: "synthetic", source: "test", collectedAt: "2026-08-14T00:00:00Z" }, tags: [] };
const response = { intent: "URL_SCAN", status: "available", trustScore: 90, riskScore: 10, confidence: .9, riskLevel: "minimal", verdict: "safe", evidence: [{ id: "e", kind: "source", title: "Known source", detail: "Observed", status: "available", weight: .8, supportsRisk: false }], limitations: [], contributors: [] } as const;
...
test("invalid or mismatched responses receive zero", () => {
  ...
  assert.equal(evaluateResponse({ expected, response: { ...response, intent: "AI_TEXT_DETECTION" } }).score, 0);
});
