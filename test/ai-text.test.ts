...
test("adversarial styles remain uncertainty-aware", async () => {
  const polishedHuman = await router.route({ intent: "AI_TEXT_DETECTION", text: "I revised this paragraph for three days. The argument is formal because the journal requires it, but the examples come from notes I made during fieldwork and from mistakes I recorded along the way." });
  const editedAi = await router.route({ intent: "AI_TEXT_DETECTION", text: "Furthermore, here's the thing: I can't pretend the answer is simple... Overall, the evidence points in one direction, but I'd still check the source twice before relying on it." });
  assert.ok((polishedHuman.confidence ?? 1) < .75); assert.ok((editedAi.confidence ?? 1) < .75);
  assert.notEqual(polishedHuman.riskLevel, "critical"); assert.notEqual(editedAi.riskLevel, "critical");
});
