...
export function parseRequest(body: unknown): MinerRequest {
  ...
  if (typeof raw.intent !== "string" || !SUPPORTED_INTENTS.includes(raw.intent as SupportedIntent)) throw new Error("unsupported or missing intent");
...
export function toAnalysisInput(request: MinerRequest): AnalysisInput {
  ...
  switch (request.intent) {
    ...
    case "AI_TEXT_DETECTION": return { intent: request.intent, text: requiredString(input.text, "text"), metadata: input.metadata as Record<string, unknown> | undefined };
