...
export interface CalibrationMetrics { evaluated: number; brierScore: number; expectedCalibrationError: number; confidenceBrierScore: number; }
export interface RankingMetrics { evaluatedPairs: number; pairwiseAccuracy: number; }
export interface ReliabilityMetrics { total: number; completed: number; failed: number; unavailable: number; coverage: number; meanLatencyMs: number; p50LatencyMs: number; p95LatencyMs: number; }
